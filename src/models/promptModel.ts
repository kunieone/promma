...
const getDataDir = (): string => {
  // 1. 优先使用环境变量
  if (process.env.PROMPT_MANAGER_DATA_DIR) {
    const dir = process.env.PROMPT_MANAGER_DATA_DIR;
    if (!existsSync(dir)) {
      try {
  ...
  // 2. 使用默认目录
  const defaultDir = join(homedir(), '.prompt-manager');
...
// 导出便捷函数
export const getAllPrompts = promptModel.getAll;
export const getPromptById = promptModel.getById;
export const createPrompt = promptModel.create;
export const updatePrompt = promptModel.update;
export const deletePrompt = promptModel.delete;
export const searchPrompts = promptModel.search;
export const clearAllPrompts = promptModel.clearAll;
export { getDataDir };
